 ...
 */
app.controller('AdGroupListController', ['$rootScope', '$scope', '$store', '$access', '$http', '$timeout', '$q', 'toaster', function ($rootScope, $scope, $store, $access, $http, $timeout, $q, toaster) {
   ...
   $scope.isgetchannelover=false;
    ...
    function initHeader() {
        ...
        getChannels().then(function(){
            getAdGroupByUserName().then(function(){
                $scope.query();
            });
        });
    ...
    function getAdGroupByUserName(){
        var userName=$.cookie("userName");
        return $http({
            url: 'api/7roadReport/getAdGroupByUserName',
            method: 'GET',
            params: {
                user_name:userName,
                game_id:$scope.nowgame
            ...
            .success(function (data, header, config, status) {
                $scope.adGroup.adGroups = data.result.map(function(ele){ele.isChecked = true;return ele;});
                $scope.adGroup.powers =  $scope.adGroup.adGroups;
            })
            .error(function (data, header, config, status) {
                pop('error', 'error', data.msg);
            });
    ...
    function drawChart(data){
        ...
        for (var i = 0; i < drawLength; i++) {
            var temp = data[i];
            if (temp['ad_group_name'] != "汇总") {
                if( $scope.viewData.isGroup == false){
                    categories.push(temp['os']+'-'+temp['ad_group_name']);
                }
                else categories.push(temp['ad_group_name']);
    ...
    var get = {
        getAdGroupData:function(params){
            $http({
                url: 'api/7roadReport/getAdGroupList',
                method: 'GET',
                params:params
            }).success(function (data, header, config, status) {
                data.result.shift();
                drawChart(data.result);
                if(params.isShowGroup == false){
                    $scope.viewData.dataTable = data.result.map(function(ele) {
                        var newItem = {};
                        newItem['推广活动'] = ele['ad_group_name'] +  ele.os;
                ...
                else $scope.viewData.dataTable =  data.result.map(function(ele) {
                    var newItem = {};
                    newItem['推广活动组'] = ele['ad_group_name'];
                    newItem['激活'] = ele['installs'];
                    newItem['注册'] = ele['regs'];
                    newItem['创角'] = ele['roles'];
                    newItem['登录'] = ele['login'];
        ...
        getAdActivityData:function(params){
            $http({
                ...
                data.result.shift();
                $scope.viewData.subDataTable = data.result;
                $('#subActivityModel').modal('show');
            }).error(function (data) {
                pop('error', '链接异常', data);
            });
        },
        exportAdGroupData:function(params){
            $http({
                url: 'api/7roadReport/export/getAdGroupList',
                method: 'GET',
                params:params
            }).success(function (data, header, config, status) {
    ...
    $scope.showSubActivity = function(item){
        if(!$scope.viewData.isGroup||item.ad_group_name == '汇总') return ;
        if($scope.viewData.currentGroup == item.ad_group_id){
            $('#subActivityModel').modal('show');
            return;
        }
        var obj = {
        ...
        $scope.viewData.currentGroup = obj.ad_group_id;
        $scope.viewData.currentGroupName = item.ad_group_name;
    ...
    $scope.query = function () {
        var ad_groups =  $scope.adGroup.powers.map(function(ele){return ele.id}).join(',');
            var obj = {
                curDate:moment($scope.initData.curDate).format('YYYY-MM-DD') ,
                date1: moment($scope.initData.rangeDate.startDate).format('YYYY-MM-DD'),
                date2: moment($scope.initData.rangeDate.endDate).format('YYYY-MM-DD'),
        ...
        if( $scope.adGroup.powers.length == 1) {
            obj.isShowGroup = false;
        }
        else obj.isShowGroup = true;
            get.getAdGroupData(obj);
        $scope.viewData.isGroup = obj.isShowGroup;
    ...
    $scope.export = function(type){
        if(type == 1) {
            ...
            var obj = {
                ...
                game_id: $scope.nowgame,
                in_os: $scope.os.selected.os,
                ad_group_id:$scope.viewData.currentGroup,
                isShowGroup:false
            };
    ...
    $scope.GruopSelectedClose = function(){
        if($scope.adGroup.powers.length == 0) {
            pop('error', '链接异常', "必须选择一个活动组");
            return;
        }
        $scope.query();
    }
