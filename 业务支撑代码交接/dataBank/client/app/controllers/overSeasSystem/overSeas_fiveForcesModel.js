 ...
 */
'use strict';
app.controller('overSeas5liDataController', ['$rootScope', '$scope', '$http', '$timeout', '$q', 'toaster', '$params', function ($rootScope, $scope, $http, $timeout, $q, toaster, $params) {
        ...
        $http({
            ...
            if (data.code == 0) {
                ...
                for (var i = 0; i < data.result.length; i++) {
                    ...
                    //}
                    else if (temp.pointertype_name == '登录用户数') {
                        todayData.loginCount.count=temp[curParams['curDate']];
                        todayData.loginCount.trend=temp.trend;
                    }
                    ...
                    //}
                        ...
                        todayData.payMoney.count=temp[curParams['curDate']];
    ...
    {
        if (curParams.datetype === 1) {
            exportData('/api/overSeas/export/5li-day');
        } else if (curParams.datetype === 2) {
            exportData('/api/overSeas/export/5li-week');
        } else {
            exportData('/api/overSeas/export/5li-month');
        }
    };
    /**
     * 刷新数据
    ...
    var refreshData = function () {
        if (curParams) {
            if (curParams.datetype === 1) {
                getData('api/overSeas/5li-day');
            } else if (curParams.datetype === 2) {
                getData('api/overSeas/5li-week');
            } else {
                getData('api/overSeas/5li-month');
            }
        } else {
