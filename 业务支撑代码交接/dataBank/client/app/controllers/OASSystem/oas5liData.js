 ...
 */
'use strict';
app.controller('oas5liDataController', ['$rootScope', '$scope', '$http', '$timeout', '$q', 'toaster', '$params', function ($rootScope, $scope, $http, $timeout, $q, toaster, $params) {
    ...
    {
        if (curParams.datetype === 1) {
            exportData('/api/oas/export/5li-day');
        } else if (curParams.datetype === 2) {
            exportData('/api/oas/export/5li-week');
        } else {
            exportData('/api/oas/export/5li-month');
        }
    };
    /**
     * 刷新数据
