...
function insertToDB(data){
    ...
    for(var i = 0; i <  data.length; i++){
        var row = data[i];
        if(row.length >= 1){
            var count_date = moment(new Date(row[0])).format('YYYY-MM-DD');
            var game_id = row[1];
            var media_source = row[2];
            var os= row[3];
            var installs = row[4] ? row[4] : 0;
            var reg_count = row[5] ? row[5] :0;
            var role_count = row[6] ? row[6]:0;
            var cost = row[7] ? row[7] :0;
            var type = row[8] ? row[8] :0; // 1为：抵消自然量(即自然量含有该渠道的数据，自然量数据统计应减去此渠道数据)
