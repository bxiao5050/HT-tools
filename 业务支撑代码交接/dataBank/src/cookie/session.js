...
SessionManager.prototype.fire = function(response){
    var sessionId = request.cookie(SESSIONID_KEY);
    var curSession;
    if (sessionId && (curSession = this.get(sessionId))) {
        if (this.isTimeout(curSession)) {
            this.remove(sessionId);
            curSession = this.renew(response);
        } else {
...
exports.SessionManager = new SessionManager(config.Timeout);
