 ...
 */
@com.dataprovider.core.annotations.DataHandler(index = 3,description = "整合系统数据")
public class IntergrateDataHandler implements DataHandler {
    public OutputModel handler(InputModel model) {
        ...
        try {
            if(!Strings.isNullOrEmpty(out.getExecuteResult())){
                String result = FutureExecutor.updateIntergrateCache(out.getCacheId(), model).get(2,TimeUnit.SECONDS);
                if(!Strings.isNullOrEmpty(result)) {
                    out.setExecuteResult(result);
                }
            }else{
                out.setExecuteResult(FutureExecutor.updateIntergrateCache(out.getCacheId(), model).get());
            }
        }catch (Exception e){
            return out;
        }
