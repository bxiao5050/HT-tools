<template>
    <div>
        <div class="lost-group-header">
            ...
            <span class="index-header-text">流失用户数据</span>
        </div>
        ...
        methods: {
            ...
            drawChart: function () {
                ...
                if (data) {
                    ...
                    var chartData = [{ name: '每日流失数', data: [], type: 'column', yAxis: 0 },
                    { name: '每日流失率', data: [], type: 'spline', tooltip: { valueSuffix: "%" }, yAxis: 1 }];
                    if (this.time_type == 7) {
                        chartData[0].name = "每日流失数";
                        chartData[1].name = "每日流失率";
                    } else if (this.time_type == 14) {
                        chartData[0].name = "每周流失数";
                        chartData[1].name = "每周流失率";
                    } else if (this.time_type == 30) {
                        chartData[0].name = "每月流失数";
                        chartData[1].name = "每月流失率";
                    }
                    for (var i = 0; i < data.length; i++) {
                        chartData[0].data.push(Number(data[i][chartData[0].name]));
                        chartData[1].data.push(Number(data[i][chartData[1].name]));
                    }
                    highchartUtil.drawTwoYaxisChart('loseChart', categories, chartData, true);
                }
            },
        },
        watch: {
