<template>
    <div>
        <div class="lost-group-header">
            ...
            <span class="index-header-text">流失用户详细分析</span>
        </div>
        ...
        <option value="1">付费玩家</option>
            ...
            <select class="player-type" v-model="config_id">
        <option value="1">流失前等级</option>
        <option value="2">付费金额</option>
        <option value="3">付费次数</option>
        <option value="4">用户生命周期(注册到流失时间段)</option>
        </select>
    ...
    export default {
        ...
        methods: {
            ...
            query: function () {
                ...
                httpRequest.getGameGKLoseDetailData(param, (data) => {
            ...
            drawChart: function () {
                ...
                if (data) {
                    ...
                    for (var i = 0; i < data.length; i++) {
                        categories.push(data[i].config_name);
                    }
                    var chartData = [];
                       var chartData = [{ name: '流失人数', data: [], type: 'column', yAxis: 0 },
                    { name: '百分比', data: [], type: 'spline', tooltip: { valueSuffix: "%" }, yAxis: 1 }];
                    for (var i = 0; i < data.length; i++) {
                        chartData[0].data.push(Number(data[i]["value"]));
                        chartData[1].data.push(Number(data[i]["rate"]));
                    }
                    highchartUtil.drawTwoYaxisChart('loseDetailChart', categories, chartData,true);
                }
            },
        },
        watch: {
            ...
            config_id: function (newVal, oldVal) {
                if (newVal && newVal != oldVal) {
                    this.query();
                }
            },
...
.player-type>select::-ms-expand { display: none; }
