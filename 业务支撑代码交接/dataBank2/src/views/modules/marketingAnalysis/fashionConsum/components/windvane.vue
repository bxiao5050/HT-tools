...
<template>
  <div>
    <!-- <table class="table table-bordered">
      <thead>
        <tr>
          <th v-for="(column,cindex) in columnArr" :key="cindex" v-if="cindex!=1">{{column}}</th>
        </tr>
      </thead>
      <tbody>
        <tr v-for="(item,index) in tableData" :key="index">
          <td v-for="(column,cindex) in columnArr" :key="cindex" v-if="cindex!=1">
            <a href="javascript:void(0)" v-if="cindex==2" @click="showDetail(item)">{{item[column]}}</a>
     ...
     <el-table ref="singleTable" :data="tableData" @row-click="rowClick" :highlight-current-row="true" max-height="500" style="width:100%;" fit border>
      <el-table-column v-for="(column,cindex) in columnArr" :key="column" :prop="column" :label="column" v-if="cindex!=1">
        <template slot-scope="scope">
          <a href="javascript:void(0)" v-if="cindex==2" @click="showDetail(scope.row)">{{scope.row[column]}}</a>
          <i v-else-if="cindex==3" :class="{'icon-arrow up':scope.row[column]>0,'icon-arrow right':scope.row[column]==0,'icon-arrow down':scope.row[column]<0}"></i>
