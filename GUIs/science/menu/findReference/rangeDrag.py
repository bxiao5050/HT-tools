...
class RangeDrag():
    def __init__(self, master, color = 'blue', ax = None, startPos = None):
        ...
        self.width = ax.get_xlim()[1]/400


        xmin, xmax = ax.get_xlim()
        ymin, ymax = ax.get_ylim()
        if startPos is None:
            startPos = np.random.randint(xmin - self.width, xmax, size = 1)
        self.rect1 = ax.bar(startPos,bottom = ymin, height = abs(ymax-ymin),  color = color, width = self.width, alpha = 0.6)[0]
        self.rect2 = ax.bar(startPos + self.width*5,bottom = ymin, height = abs(ymax-ymin),  color = color, width = self.width, alpha = 0.6)[0]

        self.middle = ax.bar((self.rect2.xy[0] + self.rect1.xy[0] + self.width)/2,bottom = ymin, height = abs(ymax-ymin),  color = color, alpha = 0.1, width = self.rect2.xy[0] - self.rect1.xy[0] - self.width)[0]
