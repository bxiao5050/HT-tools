...
class RangeDrag():
    def __init__(self, master, color = 'orange', ax = None):
        ...
        self.rect1 = ax.bar(startPos, ymax,  color = 'orange', width = self.width)[0]
        self.rect2 = ax.bar(startPos + self.width*5, ymax,  color = 'orange', width = self.width)[0]

        self.middle = ax.bar((self.rect2.xy[0] + self.rect1.xy[0] + self.width)/2, ymax,  color = color, alpha = 0.1, width = self.rect2.xy[0] - self.rect1.xy[0] - self.width)[0]
        # self.middle = ax.bar((self.rect2.xy[0] + self.rect1.xy[0])/2, ymax,  color = color, alpha = 0.1, width = self.rect2.xy[0] - self.rect1.xy[0] )[0]
        l1 = self.rect1.xy[0]
        l2 = self.rect2.xy[0]
        self.inf.config(text = '{:.1f}'.format(min(l1, l2)) + ' - ' + '{:.1f}'.format(max(l1, l2)))
