...
from mywidgets.wafer import myWafer
...
class RunWafer(Frame):
    def __init__(self,master, **kw ):
        ...
        f = Frame(self)
        f.pack(fill ='both')
        self.wf = myWafer.WaferArrange(f)
...
class RunRange(LabelFrame):
    def __init__(self,master, **kw ):
        ...
        R1 = Radiobutton(self, text="All positions", variable=self.var, value=1,
                          command=self.sel)
        R2 = Radiobutton(self, text="Choose:", variable=self.var, value=2,
                          command=self.sel)
        self.R2e = Entry(self)
        self.R2e.insert(0, '1-5, 7, 12')
        self.R2e.config(state = 'disabled')

        R3 = Radiobutton(self, text="Select on wafer", variable=self.var, value=3,
                          command=self.sel)
